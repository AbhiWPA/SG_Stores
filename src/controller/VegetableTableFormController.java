...
public class VegetableTableFormController implements Initializable {
    public TableView tblVegetables;
    ...
    public void clearBtnOnAction(ActionEvent actionEvent) {
        for ( int i = 0; i<tblVegetables.getItems().size(); i++) {
            tblVegetables.getItems().clear();
        }

        try {
            CrudUtil.execute("DELETE FROM todaysellings WHERE Item_code LIKE 'V%'");
    ...
    private void setTableDetails() throws SQLException, ClassNotFoundException {
        ResultSet result = CrudUtil.execute("SELECT * FROM todaysellings WHERE Item_code LIKE 'V%'");
